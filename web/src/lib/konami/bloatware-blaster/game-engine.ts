...
export class GameEngine {
  ...
  private handlePlayerShooting(): void {
    if (this.input.isPressed('Space') && this.input.canShoot()) {
      for (const bullet of this.bullets) {
        if (!bullet.active) {
          bullet.fire(this.player.x + this.player.width / 2 - bullet.width / 2, this.player.y)
          playShoot()
          break
        }
      }
  ...
  private moveInvaders(dt: number): void {
    ...
    for (const invader of aliveInvaders) {
      const nextX = invader.x + this.invaderSpeed * this.invaderDirection * dt * 60
    ...
    const percentRemaining = aliveInvaders.length / (INVADER_ROWS * INVADER_COLS)
    this.invaderSpeed =
      INVADER_BASE_SPEED * (1 + (1 - percentRemaining) * INVADER_SPEED_INCREASE * 5)
  }
  ...
  private checkWinLose(): void {
    ...
    if (aliveCount === 0) {
      ...
      const shieldHealth = this.shields.reduce((sum, shield) => sum + shield.health, 0)
      ...
      const timeMultiplier = Math.max(1, 2 - this.gameTime / 120)

      // Final score = (base + bonuses) * time multiplier
      this.score = Math.round((basePoints + livesBonus + shieldBonus) * timeMultiplier)
      this.onScoreChange?.(this.score)
  ...
  private render(): void {
    ...
    // Draw bullets
    for (const bullet of this.bullets) {
      this.renderer.drawBullet(bullet, 'oklch(0.92 0.35 100)')
    }
    for (const bullet of this.enemyBullets) {
      this.renderer.drawBullet(bullet, 'oklch(0.72 0.30 10)')
    }
  }
}
