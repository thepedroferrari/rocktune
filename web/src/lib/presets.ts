...
function toPresetConfig(persona: PersonaRaw): PresetConfig {
  return {
    ...
    software: persona.recommended_software.map((item) =>
      item.key as PackageKey
    ),
  };
}
...
export const PRESET_ORDER: PresetType[] = personas.map((persona) =>
  persona.id as PresetType
);
