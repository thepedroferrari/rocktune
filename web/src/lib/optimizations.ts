...
const SAFE_NETWORK: readonly OptimizationDef[] = [
  ...
  {
    key: OPTIMIZATION_KEYS.NIC_INTERRUPT_MOD,
    tier: OPTIMIZATION_TIERS.SAFE,
    category: 'network',
    label: 'NIC Interrupt Moderation Off',
    hint: 'Lower network latency',
    tooltip: {
      title: 'NIC Interrupt Moderation Off',
      desc: 'Disables interrupt coalescing — packets processed immediately',
      pros: ['Lower network latency', 'Essential for competitive gaming', 'Reduces input lag'],
      cons: ['Slightly higher CPU usage', 'Not all adapters support'],
    },
    defaultChecked: false,
    rank: EFFECTIVENESS_RANKS.A,
  },
  {
    key: OPTIMIZATION_KEYS.NIC_FLOW_CONTROL,
    tier: OPTIMIZATION_TIERS.SAFE,
    category: 'network',
    label: 'NIC Flow Control Off',
    hint: 'Disable packet buffering',
    tooltip: {
      title: 'NIC Flow Control Off',
      desc: 'Disables IEEE 802.3x flow control — prevents packet buffering',
      pros: ['No flow control delays', 'Better for gaming', 'Reduces jitter'],
      cons: ['Possible packet loss on saturated links', 'Not all adapters support'],
    },
    defaultChecked: false,
    rank: EFFECTIVENESS_RANKS.A,
  },
  {
    key: OPTIMIZATION_KEYS.NIC_ENERGY_EFFICIENT,
    tier: OPTIMIZATION_TIERS.SAFE,
    category: 'network',
    label: 'NIC Energy Efficient Ethernet Off',
    hint: 'Disable EEE/Green Ethernet',
    tooltip: {
      title: 'NIC Energy Efficient Ethernet Off',
      desc: 'Disables Energy Efficient Ethernet (EEE/Green Ethernet)',
      pros: ['No EEE wake latency', 'Consistent ping times', 'Prevents micro-stutters'],
      cons: ['Slightly higher power consumption', 'Not all adapters support'],
    },
    defaultChecked: false,
    rank: EFFECTIVENESS_RANKS.A,
  },
...
const SAFE_PRIVACY: readonly OptimizationDef[] = [
  ...
  {
    key: OPTIMIZATION_KEYS.BROWSER_BACKGROUND,
    tier: OPTIMIZATION_TIERS.SAFE,
    category: 'privacy',
    label: 'Browser Background Apps Off',
    hint: 'Stop Chrome/Edge background processes',
    tooltip: {
      title: 'Browser Background Apps Off',
      desc: 'Disables Chrome and Edge background apps via Group Policy',
      pros: [
        'Stops browsers running when closed',
        'Frees RAM and CPU',
        'No startup boost preloading',
      ],
      cons: ['Slower browser startup', 'No background extensions'],
    },
    defaultChecked: false,
    rank: EFFECTIVENESS_RANKS.A,
  },
