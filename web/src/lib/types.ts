...
function isStringEnumValue<T extends string>(
  values: readonly T[],
  value: unknown,
): value is T {
...
export type DnsProviderType =
  (typeof DNS_PROVIDERS)[keyof typeof DNS_PROVIDERS];
...
export type PeripheralType =
  (typeof PERIPHERAL_TYPES)[keyof typeof PERIPHERAL_TYPES];

const PERIPHERAL_TYPE_VALUES = Object.values(
  PERIPHERAL_TYPES,
) as PeripheralType[];
...
const MONITOR_SOFTWARE_VALUES = Object.values(
...
export function isMonitorSoftwareType(
  value: unknown,
): value is MonitorSoftwareType {
...
export function isFilterSelected(
  filter: FilterValue,
): filter is typeof FILTER_SELECTED {
  return filter === FILTER_SELECTED;
}

export function isFilterRecommended(
  filter: FilterValue,
): filter is typeof FILTER_RECOMMENDED {
...
export type OptimizationTier =
  (typeof OPTIMIZATION_TIERS)[keyof typeof OPTIMIZATION_TIERS];
...
export type EffectivenessRank =
  (typeof EFFECTIVENESS_RANKS)[keyof typeof EFFECTIVENESS_RANKS];
...
export type OptimizationKey =
  (typeof OPTIMIZATION_KEYS)[keyof typeof OPTIMIZATION_KEYS];

export type SafeOptimization =
  (typeof SAFE_OPTIMIZATIONS)[keyof typeof SAFE_OPTIMIZATIONS];
export type CautionOptimization =
  (typeof CAUTION_OPTIMIZATIONS)[keyof typeof CAUTION_OPTIMIZATIONS];
export type RiskyOptimization =
  (typeof RISKY_OPTIMIZATIONS)[keyof typeof RISKY_OPTIMIZATIONS];
export type LudicrousOptimization =
  (typeof LUDICROUS_OPTIMIZATIONS)[keyof typeof LUDICROUS_OPTIMIZATIONS];

const OPTIMIZATION_KEY_VALUES = Object.values(
  OPTIMIZATION_KEYS,
) as OptimizationKey[];
...
/** Map tools to applicable personas */
export const TOOL_PERSONA_MAP: Record<ConfigTool, readonly PresetType[]> = {
  [CONFIG_TOOLS.OBS]: ["streamer"],
  [CONFIG_TOOLS.MSI_AFTERBURNER]: ["gamer", "pro_gamer", "benchmarker"],
  [CONFIG_TOOLS.FAN_CONTROL]: ["gamer", "pro_gamer", "benchmarker"],
  [CONFIG_TOOLS.UNIFI]: ["pro_gamer"],
  [CONFIG_TOOLS.NVIDIA_INSPECTOR]: [
    "gamer",
    "pro_gamer",
    "benchmarker",
    "streamer",
