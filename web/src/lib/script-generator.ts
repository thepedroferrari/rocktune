...
function hasNonSafeOptimizations(selected: Set<OptimizationKey>): boolean {
  for (const key of selected) {
    ...
    if (
      tier &&
      TIER_PRIORITY[tier] >= TIER_PRIORITY[OPTIMIZATION_TIERS.CAUTION]
    ) {
      return true;
    }
  }
...
export function buildScript(
  ...
  lines.push(
    `${indent}Write-Host "  RAM: ` + "$ram" + `GB" -ForegroundColor White`,
  );
  lines.push(
...
function generateNetworkOpts(
  ...
  if (selected.has("dns")) {
    ...
    const [primary, secondary] =
      dnsServers[dnsProvider] || dnsServers.cloudflare;
